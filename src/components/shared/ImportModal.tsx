...
interface ImportModalProps {
  ...
  onClose: () => void;
  type: ImportType;
  onImport: (csvContent: string, accountId?: number, skipDuplicates?: boolean, dateFormat?: string) => Promise<{ count: number; message: string }>;
  accounts?: Account[];
  dateFormat?: string;
}
...
export const ImportModal: React.FC<ImportModalProps> = ({ isOpen, onClose, type, onImport, accounts, dateFormat }) => {
  ...
  useEffect(() => {
    if (isOpen) {
      ...
      setError(null);
      setSuccessMsg(null);
      setSelectedAccountId(accounts && accounts.length > 0 ? accounts[0].id : null);
      setDuplicates([]);
      ...
      if (type === 'categories') {
        ...
        rows = lines
          .filter((l) => {
            ...
            return !low.startsWith('date') && !low.startsWith('data') && !low.startsWith('trans');
          })
          .map((l) => {
            const parts = l.split(';');
            if (parts.length < 6) return null;
            const [date, payType, num, payee, memo, amount, catName] = parts.map((s) => s.trim());
        ...
        rows = lines
          .filter((l) => {
            const low = l.toLowerCase();
            return !low.startsWith('payee') && !low.startsWith('name') && !low.startsWith('kontrahent');
          })
          .map((l) => {
            const parts = l.split(';');
  ...
  const executeImport = async () => {
    ...
    try {
       const result = await onImport(preview.rawContent, selectedAccountId || undefined, !ignoreDuplicates, dateFormat);
  ...
  const visibleRows = preview?.rows.filter(row => {
    ...
    if (type === 'transactions') {
        return !duplicates.some(d => d.date === row.date && d.payee === row.payee && Math.abs(parseFloat(row.amount.replace(',','.')) - d.amount) < 0.001);
    }
    if (type === 'payees') {
  ...
  return (
    <div className="fixed inset-0 z-[1000] flex items-start md:items-center justify-center p-2 md:p-4 pt-[2vh] md:pt-4 backdrop-blur-md bg-slate-950/80 animate-in fade-in duration-300">
      <div className="bg-slate-900 border border-slate-800 p-5 md:p-8 rounded-[2rem] md:rounded-[2.5rem] shadow-2xl w-full max-w-4xl max-h-[92vh] md:max-h-[85vh] flex flex-col space-y-4 md:space-y-6 relative overflow-hidden">
        ...
        <div className="flex justify-between items-start shrink-0">
          ...
          <button onClick={onClose} className="p-2 md:p-3 bg-slate-800 rounded-full hover:bg-slate-700 transition-all">
            ...
            <div className="flex-1 flex flex-col items-center justify-center border-2 border-dashed border-slate-700 rounded-2xl p-8 md:p-12 gap-4 hover:border-indigo-500/50 transition-colors bg-slate-950/50 min-h-[300px]">
             ...
             <div className="flex-1 overflow-hidden flex flex-col gap-3 md:gap-4">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-3 md:gap-4 bg-slate-800/30 p-4 rounded-2xl border border-slate-800/50">
                        ...
                        <div className={`space-y-1.5 ${type !== 'transactions' ? 'md:col-span-2' : ''}`}>
                             <label className="text-[9px] font-black uppercase tracking-widest text-slate-500">Duplicate Check</label>
                             {isCheckingDuplicates ? (
                                ...
                                <div className="flex items-center justify-between gap-4">
                                    <span className="text-[10px] md:text-xs text-rose-400 font-bold uppercase">{duplicates.length} Found</span>
                                    <label className="flex items-center gap-2 cursor-pointer group">
                                        ...
                                        />
                                        <span className="text-[9px] font-black text-slate-400 group-hover:text-white transition-colors uppercase tracking-tight">Import anyway?</span>
                                    </label>
                                </div>
                             ) : (
                                <div className="text-[10px] md:text-xs text-emerald-400 font-bold uppercase">No duplicates detected</div>
                             )}
                        </div>
                    </div>
                      ...
                      <table className="w-full text-left text-[10px] md:text-xs border-collapse">
                          <thead className="sticky top-0 bg-slate-900 border-b border-slate-800 text-slate-500 font-black uppercase tracking-wider z-10 shadow-sm">
                              <tr className="divide-x divide-slate-800/30">
                                  {type === 'categories' && <>
                                      <th className="p-2 md:p-3 w-12 text-center">Lvl</th><th className="p-2 md:p-3 w-12 text-center">T</th><th className="p-2 md:p-3">Name</th>
                                  </>}
                                  {type === 'transactions' && <>
                                      <th className="p-2 md:p-3 w-20 md:w-28">Date</th><th className="p-2 md:p-3">Payee</th><th className="p-2 md:p-3 w-16 md:w-24 text-right">Amt</th><th className="p-2 md:p-3 hidden sm:table-cell">Cat</th>
                                  </>}
                                  {type === 'payees' && <>
                                       <th className="p-2 md:p-3">Name</th><th className="p-2 md:p-3">Cat</th><th className="p-2 md:p-3 hidden sm:table-cell">Mode</th>
                                  </>}
                              </tr>
                          </thead>
                          <tbody className="divide-y divide-slate-800/50 text-slate-300">
                              {visibleRows.slice(0, 100).map((row, i) => {
                                  let isDup = false;
                                  if (type === 'transactions') {
                                      isDup = duplicates.some(d => d.date === row.date && d.payee === row.payee && Math.abs(parseFloat(row.amount.replace(',','.')) - d.amount) < 0.001);
                                  } else if (type === 'payees') {
                                      isDup = duplicates.some(d => d.name === row.name);
                                  }
                                  return (
                                     <tr key={i} className={`hover:bg-slate-800/50 divide-x divide-slate-800/30 ${isDup ? 'bg-rose-500/5' : ''}`}>
                                         {type === 'categories' && <>
                                             <td className="p-2 md:p-3 text-center">{row.level}</td>
                                             <td className="p-2 md:p-3 text-indigo-400 font-bold text-center">{row.type}</td>
                                             <td className="p-2 md:p-3 truncate">{row.name}</td>
                                         </>}
                                         {type === 'transactions' && <>
                                             <td className="p-2 md:p-3 opacity-70 flex items-center gap-2 truncate">
                                                 {isDup && <span className="w-1.5 h-1.5 rounded-full bg-rose-500 shrink-0" title="Duplicate Found"></span>}
                                                 {row.date}
                                             </td>
                                             <td className="p-2 md:p-3 font-bold truncate">{row.payee}</td>
                                             <td className={`p-2 md:p-3 font-bold text-right ${parseFloat(row.amount?.replace(',','.') || 0) > 0 ? 'text-emerald-400' : 'text-slate-300'} truncate`}>{row.amount}</td>
                                             <td className="p-2 md:p-3 opacity-70 truncate hidden sm:table-cell">{row.catName}</td>
                                         </>}
                                         {type === 'payees' && <>
                                             <td className="p-2 md:p-3 font-bold truncate">{row.name}</td>
                                             <td className="p-2 md:p-3 opacity-70 truncate">{row.catName}</td>
                                             <td className="p-2 md:p-3 opacity-70 truncate hidden sm:table-cell">{row.payModeName}</td>
                                         </>}
                                     </tr>
                                  );
                              })}
                 ...
                 <div className="flex gap-3 pt-1 shrink-0">
                     <Button
                        ...
                        disabled={(type === 'transactions' && !selectedAccountId) || (type === 'transactions' && duplicates.length > 0 && !ignoreDuplicates && visibleRows.length === 0)}
