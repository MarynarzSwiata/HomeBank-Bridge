...
export const PayeeForm: React.FC<PayeeFormProps> = ({
  ...
  useEffect(() => {
    if (editingPayee) {
      ...
      setDefaultPaymentType(editingPayee.default_payment_type !== null ? String(editingPayee.default_payment_type) : '');
    } else {
      setName('');
      setMainCategoryId('');
      setSubCategoryId('');
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      const payload = {
        ...
        defaultPaymentType: defaultPaymentType ? parseInt(defaultPaymentType) : null
      };
  ...
  return (
    <div className={`space-y-6 transition-all duration-500 ease-in-out ${isExpanded ? "opacity-100" : "opacity-0 pointer-events-none h-0 p-0 overflow-hidden"}`}>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            <div>
                 <label className="text-[9px] font-black uppercase tracking-widest text-slate-500 ml-2 mb-1.5 block">Payee Name</label>
                 <input 
