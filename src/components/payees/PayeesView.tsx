...
import { triggerDownload, sanitizeCSVField, getExportDateString } from '../../utils/exportUtils';
  ...
  onToggleAnonymize
    ...
    const filteredPayees = useMemo(() => {
        let result = payees.filter(p => {
            const catPath = p.default_category_id ? categoryMap.get(p.default_category_id) : '';
            return p.name.toLowerCase().includes(filterSearch.toLowerCase()) ||
        ...
        result.sort((a, b) => {
            ...
            switch(sortField) {
                ...
                case 'category': 
                    valA = a.default_category_id ? categoryMap.get(a.default_category_id) || '' : '';
                    valB = b.default_category_id ? categoryMap.get(b.default_category_id) || '' : '';
                    break;
                case 'amount': valA = a.total_amount || 0; valB = b.total_amount || 0; break;
                case 'usage': valA = a.count || 0; valB = b.count || 0; break;
    ...
    const handleImport = async (csvContent: string, _accountId?: number, skipDuplicates?: boolean) => {
  ...
  return (
    <div className="animate-in fade-in slide-in-from-bottom-8 duration-700 space-y-12 max-w-5xl mx-auto">
      ...
      <div className="space-y-4">
        <ActionBar
          title={editingPayee ? 'Edit Payee' : isFormExpanded ? 'New Payee' : 'Entity Management'}
          subtitle={editingPayee ? `Modifying ${editingPayee.name}` : 'Manage your frequent payees'}
          ...
          actions={
            <div className="flex gap-2">
              ...
              <Button
                ...
                icon={<svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M4 16v1a3 3 0 003 3h10a3 3 0 003-3v-1m-4-8l-4-4m0 0L8 8m4-4H4" /></svg>}
              >
                Export
              </Button>
            </div>
          }
          className={editingPayee ? 'border-indigo-500/50 bg-indigo-950/10' : ''}
        />

        {(isFormExpanded || editingPayee) && (
           <Card variant="default" className={editingPayee ? 'border-indigo-500/50 bg-indigo-950/5' : ''}>
             <PayeeForm 
                isExpanded={true}
       ...
       <ImportModal
        ...
        <div className="bg-slate-900/50 border border-slate-800 rounded-3xl p-6">
            <div className="relative group">
                <label className="block text-[10px] font-black uppercase tracking-widest text-slate-500 mb-2 px-2">Refine Dictionary</label>
        ...
        <div className="flex flex-col md:flex-row justify-between items-stretch md:items-center gap-4 px-4 pb-4">
            <div className="flex flex-wrap items-center gap-x-6 gap-y-3 text-[10px] font-black uppercase tracking-widest text-slate-600">
                ...
                <div className="flex items-center gap-2">
                    ...
                    Viewing: <span className="text-slate-400">{filteredPayees.length}</span>
                </div>
                {selectedIds.size > 0 && (
                ...
                <div className="flex items-center gap-2">
                    ...
                    >
                        ...
                        <span className="font-black text-[10px] hidden sm:inline">{isAnonymized ? 'HIDDEN' : 'VISIBLE'}</span>
                    </button>
            ...
            {selectedIds.size > 0 && (
                <Button
                    variant="danger"
                    size="sm"
                    onClick={() => setConfirmDelete({ id: -1, name: `${selectedIds.size} entities` })}
        ...
        <div className="space-y-4">
            ...
            <div className="hidden md:grid grid-cols-12 gap-4 px-8 py-5 text-[10px] font-black uppercase tracking-[0.2em] text-slate-500 bg-slate-900/30 rounded-3xl border border-slate-800/50">
                <div className="col-span-1 flex items-center justify-center">
                    <input 
                        type="checkbox" 
                        checked={filteredPayees.length > 0 && filteredPayees.slice(0, itemsToShow).every(p => selectedIds.has(p.id))}
                        onChange={toggleAll}
                ...
                <SortableHeader field="name" label="Payee Name" colSpan="col-span-3 !justify-start" />
                <SortableHeader field="category" label="Default Category" colSpan="col-span-2" />
                <div className="col-span-2 text-center">Payout Method</div>
                <SortableHeader field="amount" label="Total Spent" colSpan="col-span-1 font-black text-indigo-400/80" />
            ...
            {filteredPayees.length === 0 ? (
                <div className="text-center py-24 bg-slate-900/20 border-2 border-dashed border-slate-800 rounded-[3rem]">
                    ...
                    <p className="text-lg font-black uppercase tracking-widest text-slate-500">No Entities Found</p>
                    <p className="text-xs font-bold text-slate-700 uppercase mt-2">Try refining your search terms</p>
                </div>
            ) : (
                <div className="space-y-3">
                    {filteredPayees.slice(0, itemsToShow).map(payee => {
                        const isSelected = selectedIds.has(payee.id);
                        const catPath = payee.default_category_id ? categoryMap.get(payee.default_category_id) : null;
                        ...
                        return (
                            ...
                            >
                                {/* Checkbox & Name - Mobile Header */}
                                <div className="col-span-12 md:col-span-10 flex items-center justify-between md:justify-start gap-4">
                                    <div className="flex items-center gap-4 min-w-0 flex-1">
                                        <input 
                                            type="checkbox" 
                                            checked={isSelected}
                                            onChange={() => toggleSelection(payee.id)}
                                            className="w-6 h-6 md:w-5 md:h-5 rounded-lg border-slate-700 bg-slate-800 text-indigo-600 focus:ring-offset-slate-900 transition-all cursor-pointer shrink-0"
                                        />
                                        <div className="flex flex-col min-w-0 flex-1">
                                            <div className="flex items-center gap-2">
                                                <span className="text-sm md:text-sm font-black text-slate-100 uppercase tracking-tight truncate">
                                                    {payee.name}
                                                </span>
                                                <span className="text-[10px] font-black text-slate-600 shrink-0">({payee.count || 0})</span>
                                            </div>
                                            {/* Subline for Mobile (Category & Method) */}
                                            <div className="md:hidden flex flex-wrap gap-x-2 gap-y-1 mt-0.5">
                                                {catPath && (
                                                    <span className="text-[9px] font-black uppercase text-indigo-400/70 tracking-widest bg-indigo-500/5 px-2 py-0.5 rounded border border-indigo-500/10 truncate max-w-[120px]">
                                                        {catPath.split(':').pop()}
                                                    </span>
                                                )}
                                                {payee.default_payment_type !== null && (
                                                    <span className="text-[9px] font-black uppercase text-slate-500 tracking-widest bg-slate-800/50 px-2 py-0.5 rounded border border-slate-700/50">
                                    ...
                                    <div className="hidden md:flex items-center gap-8 ml-auto">
                                            ...
                                            {catPath ? (
                                                <div className="flex flex-col gap-0.5 items-center">
                                                    {catPath.split(':').map((part, i, arr) => (
                                                        <span key={i} className={`text-[9px] font-black uppercase tracking-widest truncate max-w-full ${i === arr.length - 1 ? 'text-indigo-400 bg-indigo-500/5 px-2 py-0.5 rounded-md border border-indigo-500/10' : 'text-slate-600'}`}>
                                                            {part}
                                                        </span>
                                                    ))}
                                                </div>
                                        ...
                                        {/* Method */}
                                        <div className="w-32 text-center text-[10px] font-black uppercase text-slate-500 tracking-widest">
                                            {payee.default_payment_type !== null ? PAYMENT_LEXICON[payee.default_payment_type]?.name : <span className="text-slate-700">-</span>}
                                        </div>
                                    </div>

                                    {/* Price and Actions for Mobile */}
                                    <div className="md:hidden flex flex-col items-end gap-2 shrink-0 ml-2">
                                        <div className={`text-sm font-black tracking-tight ${(payee.total_amount || 0) < 0 ? 'text-rose-400' : 'text-emerald-400'}`}>
                                            {isAnonymized ? 'XXXX' : (typeof payee.total_amount === 'number' ? payee.total_amount.toFixed(2) : '0.00')}
                                        </div>
                                        <div className="flex gap-2">
                                            <button onClick={() => startEditing(payee)} className="w-8 h-8 rounded-lg bg-slate-800 text-slate-400 flex items-center justify-center"><svg className="w-3.5 h-3.5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="3" d="M15.232 5.232l3.536 3.536m-2.036-5.036a2.5 2.5 0 113.536 3.536L6.5 21.036H3v-3.572L16.732 3.732z" /></svg></button>
                                            <button onClick={() => setConfirmDelete({ id: payee.id, name: payee.name })} className="w-8 h-8 rounded-lg bg-slate-800 text-slate-400 flex items-center justify-center"><svg className="w-3.5 h-3.5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="3" d="M19 7l-.867 12.142A2 2 0 0116.138 21H7.862a2 2 0 01-1.995-1.858L5 7m5 4v6m4-6v6m1-10V4a1 1 0 00-1-1h-4a1 1 0 00-1 1v3M4 7h16" /></svg></button>
                                        </div>
                                    </div>
                                </div>

                                {/* Desktop Usage & Actions (hidden on mobile) */}
                                <div className="hidden md:flex col-span-2 justify-end items-center gap-6 opacity-0 group-hover:opacity-100 transition-all">
                                    <div className={`text-xs font-black tracking-tight ${(payee.total_amount || 0) < 0 ? 'text-rose-400' : 'text-emerald-400'}`}>
                                        {isAnonymized ? 'XXXX' : (typeof payee.total_amount === 'number' ? payee.total_amount.toFixed(2) : '0.00')}
                                    </div>
                                    <div className="flex gap-2">
                                        <button onClick={() => startEditing(payee)} className="w-9 h-9 rounded-xl bg-slate-800 text-slate-400 hover:bg-indigo-500 hover:text-white transition-all flex items-center justify-center"><svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2.5" d="M15.232 5.232l3.536 3.536m-2.036-5.036a2.5 2.5 0 113.536 3.536L6.5 21.036H3v-3.572L16.732 3.732z" /></svg></button>
                                        <button onClick={() => setConfirmDelete({ id: payee.id, name: payee.name })} className="w-9 h-9 rounded-xl bg-slate-800 text-slate-400 hover:bg-rose-500 hover:text-white transition-all flex items-center justify-center"><svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2.5" d="M19 7l-.867 12.142A2 2 0 0116.138 21H7.862a2 2 0 01-1.995-1.858L5 7m5 4v6m4-6v6m1-10V4a1 1 0 00-1-1h-4a1 1 0 00-1 1v3M4 7h16" /></svg></button>
                                    </div>
                                </div>
                            </div>
                        );
                     ...
                     {filteredPayees.length > itemsToShow && (
                        <div className="flex justify-center pt-8">
                             ...
                             >
                                Load more payees ({filteredPayees.length - itemsToShow} remaining)
                             </Button>
                        </div>
                    )}
                </div>
        ...
        <ConfirmModal
            ...
            message={
              <>
                ...
                {confirmDelete?.id === -1 ? "This action is permanent and will remove them from the dictionary." : "This action is permanent and will remove them from the dictionary."}
              </>
            }
            confirmLabel={confirmDelete?.id === -1 ? `Burn ${selectedIds.size} Records` : "Confirm Erasure"}
