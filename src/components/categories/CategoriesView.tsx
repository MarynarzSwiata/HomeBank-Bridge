...
export const CategoriesView: React.FC<CategoriesViewProps> = ({
  ...
  onExportLogged
    ...
    const categoryStats = useMemo(() => {
        ...
        const getUsage = (c: Category): number => {
            let total = c.usage_count || 0;
            if (c.children) {
                total += c.children.reduce((acc, child) => acc + getUsage(child), 0);
            }
            totalUsageMap.set(c.id, total);
            return total;
        };
        categories.forEach(c => getUsage(c));
    ...
    const filteredResults = useMemo(() => {
        ...
        let filtered = flatCategories.filter(c => 
    ...
    const CategoryRow = ({ cat, isChild = false }: { cat: Category; isChild?: boolean; key?: any }) => {
        const isSelected = selectedIds.has(cat.id);
        const usage = categoryStats.get(cat.id) || 0;
            ...
            <div className="space-y-1">
                <div 
                    className={`group grid grid-cols-12 md:grid-cols-12 gap-4 px-6 md:px-8 py-4 rounded-[2rem] md:rounded-[2.5rem] bg-slate-900/40 border transition-all items-center ${
                        isSelected ? 'border-indigo-500 bg-indigo-500/5' : 'border-slate-800/80 hover:border-slate-700 hover:bg-slate-800/60'
                ...
                >
                    ...
                    <div className="col-span-12 md:col-span-8 flex items-center gap-4">
                        ...
                        />
                         <span className={`w-8 h-8 rounded-xl flex items-center justify-center shrink-0 font-black text-xs ${
                            cat.type === "+" ? "bg-emerald-500/10 text-emerald-500" : "bg-rose-500/10 text-rose-500"
                        }`}>
                            {cat.type}
                        </span>
                        <div className="flex flex-wrap items-baseline gap-x-2">
                            <span className={`text-sm md:text-sm font-black uppercase tracking-tight ${isChild ? 'text-slate-300' : 'text-white'}`}>
                                {cat.name}
                            </span>
                            <span className={`text-[10px] font-black tracking-widest ${usage > 0 ? 'text-indigo-400/80' : 'text-slate-600'}`}>
                                ({usage})
                            </span>
                        </div>
                    </div>
                    ...
                    <div className="col-span-6 md:col-span-2 flex justify-end gap-2 md:opacity-0 md:group-hover:opacity-100 transition-all transform md:translate-x-1 md:group-hover:translate-x-0">
                        <button onClick={() => startEditingCat(cat)} className="w-10 h-10 md:w-9 md:h-9 rounded-xl bg-slate-800 text-slate-400 hover:bg-indigo-600 hover:text-white transition-all flex items-center justify-center">
                            <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="3" d="M15.232 5.232l3.536 3.536m-2.036-5.036a2.5 2.5 0 113.536 3.536L6.5 21.036H3v-3.572L16.732 3.732z" /></svg>
                        </button>
                        <button onClick={() => requestDelete(cat)} className="w-10 h-10 md:w-9 md:h-9 rounded-xl bg-slate-800 text-slate-400 hover:bg-rose-600 hover:text-white transition-all flex items-center justify-center">
                            <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="3" d="M19 7l-.867 12.142A2 2 0 0116.138 21H7.862a2 2 0 01-1.995-1.858L5 7m5 4v6m4-6v6m1-10V4a1 1 0 00-1-1h-4a1 1 0 00-1 1v3M4 7h16" /></svg>
                        </button>
                    </div>
                </div>
                {cat.children && cat.children.length > 0 && cat.children.map(sub => (
  ...
  return (
    <div className="space-y-8 max-w-5xl mx-auto pb-20">
      ...
      <div className="animate-in fade-in slide-in-from-bottom-8 duration-700 space-y-4">
        <ActionBar
          title={editingCategory ? 'Edit Category' : isCatFormExpanded ? 'New Category' : 'Category Dictionary'}
          subtitle={editingCategory ? `Modifying ${editingCategory.name}` : 'Organize your financial taxonomy'}
          ...
          className={editingCategory ? 'border-indigo-500/50 bg-indigo-950/10' : ''}
        />

        {(isCatFormExpanded || editingCategory) && (
           <Card variant="default" className={editingCategory ? 'border-indigo-500/50 bg-indigo-950/5' : ''}>
             <CategoryForm 
                isExpanded={true}
      ...
      <div className="animate-in fade-in slide-in-from-bottom-6 duration-700 flex flex-col lg:flex-row gap-4 items-stretch lg:items-end">
          <div className="flex-1 space-y-2">
            <label className="text-[10px] font-black uppercase tracking-widest text-slate-500 ml-2">Search Taxonomy</label>
          ...
          <div className="flex flex-col sm:flex-row gap-4">
              <div className="bg-slate-900/50 border border-slate-800 rounded-3xl px-8 h-[60px] flex items-center justify-between sm:justify-start gap-4 flex-1">
                  <div className="flex items-center gap-3">
                      ...
                      <span className="text-sm font-black text-indigo-400">{flatCategories.length}</span>
                  </div>
    ...
    <div className="animate-in fade-in slide-in-from-bottom-4 duration-700 space-y-3">
        ...
        <div className="hidden md:grid grid-cols-12 gap-4 px-8 py-5 text-[10px] font-black uppercase tracking-[0.2em] text-slate-500 bg-slate-900/30 rounded-3xl border border-slate-800/50">
            <div className="col-span-1 flex items-center justify-center">
                <input 
                    type="checkbox"
                    checked={selectedIds.size > 0 && selectedIds.size === (filterText ? filteredResults : flatCategories).length}
                    onChange={toggleAll}
                />
            </div>
            <div 
                className={`col-span-1 flex items-center gap-2 ${filterText ? 'cursor-pointer hover:text-white' : ''} transition-colors`}
                onClick={() => filterText && handleSort('type')}
            >
                Type
                {filterText && sortField === 'type' && (
                    <svg className={`w-3 h-3 text-indigo-400 ${sortOrder === 'desc' ? 'rotate-180' : ''}`} fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="3" d="M19 9l-7 7-7-7" /></svg>
                )}
            </div>
            <div 
                className={`col-span-6 flex items-center gap-2 ${filterText ? 'cursor-pointer hover:text-white' : ''} transition-colors`}
        ...
        {filterText ? (
            ...
            <div className="space-y-3">
                {filteredResults.length === 0 ? (
                    <div className="text-center py-20 bg-slate-900/20 border-2 border-dashed border-slate-800 rounded-[3rem]">
                        <p className="text-lg font-black uppercase tracking-widest text-slate-500">Zero matches found</p>
                    </div>
                ) : (
                    filteredResults.map(cat => {
                         ...
                         const usage = categoryStats.get(cat.id) || 0;
                         return (
                            <div key={cat.id} className={`group grid grid-cols-12 gap-4 px-6 md:px-8 py-4 rounded-[2rem] md:rounded-[2.5rem] bg-slate-900/40 border transition-all items-center ${
                                isSelected ? 'border-indigo-500 bg-indigo-500/5' : 'border-slate-800/80 hover:border-slate-700'
                            }`}>
                                {/* Checkbox, Icon, Name and Usage */}
                                <div className="col-span-12 md:col-span-10 flex items-center gap-4">
                                    <input type="checkbox" checked={isSelected} onChange={() => toggleSelection(cat.id)} className="w-6 h-6 md:w-5 md:h-5 rounded-lg border-slate-700" />
                                    <span className={`w-8 h-8 rounded-xl flex items-center justify-center shrink-0 font-black text-xs ${cat.type === "+" ? "bg-emerald-500/10 text-emerald-500" : "bg-rose-500/10 text-rose-500"}`}>{cat.type}</span>
                                    <div className="flex flex-col">
                                        <div className="flex items-baseline gap-2">
                                            <span className="text-sm font-black text-white uppercase">{cat.name}</span>
                                            <span className="text-[10px] font-black text-indigo-400/80">({usage})</span>
                                        </div>
                                        <span className="text-[9px] font-black text-slate-600 uppercase tracking-widest truncate">{cat.fullPath}</span>
                                    </div>
                                </div>

                                <div className="col-span-6 md:col-span-2 flex justify-end gap-2 md:opacity-0 md:group-hover:opacity-100 transition-all">
                                    <button onClick={() => startEditingCat(cat)} className="w-10 h-10 md:w-9 md:h-9 rounded-xl bg-slate-800 text-slate-400 hover:text-white"><svg className="w-4 h-4 mx-auto" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path d="M15.232 5.232l3.536 3.536m-2.036-5.036a2.5 2.5 0 113.536 3.536L6.5 21.036H3v-3.572L16.732 3.732z" strokeWidth="3" /></svg></button>
                                </div>
                            </div>
                         );
                    })
            ...
            <div className="space-y-3">
                {categories.length === 0 ? (
                    <div className="text-center py-20 bg-slate-900/20 border-2 border-dashed border-slate-800 rounded-[3rem]">
                        <p className="text-lg font-black uppercase tracking-widest text-slate-500">Empty Dictionary</p>
                    </div>
                ) : (
      ...
      <ConfirmModal 
        ...
        title={confirmDelete?.id === -1 ? "Mass Erase Categories?" : "Delete Category?"}
        message={
          <>
            Are you sure you want to delete <span className="text-white font-bold">{confirmDelete?.name}</span>? 
            {confirmDelete?.id === -1 ? " All associated transaction links for these categories will be removed." : " All associated transaction tags and subcategories will be unlinked."}
            This action cannot be undone.
          </>
        }
        confirmLabel={confirmDelete?.id === -1 ? "Erase Selected" : "Confirm Erasure"}
